...
imap(data_main_analysis,
     function(data, cohort_names) {
       ...
       ggsave(plot = plots,
              filename = paste0("Survival_probability_plot_", cohort_names, ".png"),
...
mfull_males <- imap_dfr(data_main_analysis,
                        ~ tidy(coxph(Surv(years_until_death_or_censoring, mortality) ~ exposure + VEK + discharge_year + strata(strata), 
                                     subset = POHL == 1,
                                     data = .x),
                               conf.int = TRUE,
                               exponentiate = TRUE) %>%
...
mfull_females <- imap_dfr(data_main_analysis[names(data_main_analysis) != "Prostate disorders"],
                          ~ tidy(coxph(Surv(years_until_death_or_censoring, mortality) ~ exposure + VEK + discharge_year + strata(strata), 
                                       subset = POHL == 2,
                                       data = .x),
                                 conf.int = TRUE,
...
mfull_males %>%
  ...
  write.csv(file = "/path/Comorbidity_SMI/Results/HR_main_analysis_sex_stratified.csv",
            row.names = FALSE)